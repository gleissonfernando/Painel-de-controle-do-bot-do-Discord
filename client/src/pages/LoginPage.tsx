...
export default function LoginPage() {
  ...
  const features = [
    { icon: <Shield size={20} className="text-primary" />, title: t("login.features.autoMod"), desc: t("login.features.autoModDesc") },
    { icon: <Bell size={20} className="text-primary" />, title: t("login.features.socialNotif"), desc: t("login.features.socialNotifDesc") },
    { icon: <Zap size={20} className="text-primary" />, title: t("login.features.commands"), desc: t("login.features.commandsDesc") },
  ];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 relative z-10">
        ...
        <div className="w-full max-w-sm">
          <div className="bg-card border border-border rounded-xl p-8 shadow-2xl shadow-black/50">
            <h2 className="text-lg font-semibold text-foreground mb-1">{t("login.welcome")}</h2>
            <p className="text-sm text-muted-foreground mb-6">{t("login.signIn")}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-10 w-full max-w-2xl">
          {features.map((f) => (
            <div key={f.title} className="bg-card/50 border border-border rounded-lg p-4 text-center">
      ...
      <footer className="text-center py-4 text-xs text-muted-foreground relative z-10 flex items-center justify-center gap-4">
        <span>BotPanel &copy; {new Date().getFullYear()} — {t("login.footer")}</span>
