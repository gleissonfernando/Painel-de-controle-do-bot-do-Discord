...
async function checkServices() {
  ...
  try {
    ...
    currentStatus["Dashboard"].cpu = Math.round((cpu.user + cpu.system) / 1000000);
    currentStatus["Dashboard"].ram = Math.round(memory.rss / 1024 / 1024);
...
export function startMonitor() {
  if (process.env.ENABLE_MONITOR !== "true") {
    console.log("[Monitor] Skipping startup because ENABLE_MONITOR is not true.");
    return;
  }

  checkServices();
  const intervalMs = Number(process.env.MONITOR_INTERVAL_MS || 300000);
